...
<<setup, include=FALSE>>=
library("knitr")
opts_chunk$set(fig.path='figure/latex-', cache.path='cache/latex-', cache=FALSE, size="small", out.columns=84, fig.align='center', fig.show='hold')
...
s(tenChooseThreePow20=count^20,count) * 'math.pow(count, 20) == tenChooseThreePow20'
@
...
seed <- 123L
rng <- s(seed) ^ 'new scala.util.Random(seed)'
rng <- s(y=seed) ^ 'new scala.util.Random(y)'
rng <- s$.new_scala.util.Random(seed)
@
...
<<nullAsArgument>>=
s$java.lang.System.setProperties(s$.null_java.util.Properties())
@
...
fibSeqAsInt <- s$Array(0L, 1L, 1L, 2L, 3L, 5L, 8L, 13L, 21L)
@
...
fibSeqAsDouble <- s$'Array[Double]'(0L, 1L, 1L, 2L, 3L, 5L, 8L, 13L, 21L)
@
...
fibSeqAsDouble <- s$Array(0, 1, 1, 2, 3, 5, 8, 13, 21)
@
...
<<bellUsingR>>=
bell.version1b <- function(n=scalaType("I0")) {
  scalaType("D0")
  if ( n <= 0 ) stop("'n' must be at least 1.")
  if ( n == 1 ) return(1)
  r1 <- numeric(n)
